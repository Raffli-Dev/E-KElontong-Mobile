...
public class AdapterKategoriBarang  extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private final int VIEW_ITEM = 1;
    private final int VIEW_PROG = 0;

    private List<KategoriBarang> items = new ArrayList<>();
    ...
    private AdapterListener<KategoriBarang> listener;

    public void setListener(AdapterListener<KategoriBarang> listener) {
        this.listener = listener;
    }
    // Provide a suitable constructor (depends on the kind of dataset)
    public AdapterKategoriBarang(Context context, RecyclerView view, int page) {
        this.ctx = context;
        this.page = page;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_ITEM) {
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.activity_kategori_barang_row, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof OriginalViewHolder) {
            ...
            v.txt_nama.setText(kategoriBarang.category_name);


            v.lytParent.setOnClickListener(view -> {
                if (listener == null) return;
                listener.onClick(view, null, kategoriBarang, position);
            });

            v.bt_remove.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    requestDelete(kategoriBarang.id);
                }
            });
            ...
            v.bt_edit.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    ActivityKategoriBarangCreate.navigate(ctx, kategoriBarang);
                }
            });
    ...
    public void insertData(List<KategoriBarang> items) {
        setLoaded();
    ...
    private void requestDelete(String id) {
        new Request(ctx).delete_kategori_barang(id, new RequestListener<RespDeleteKategoriBarang>() {
            @Override
            public void onSuccess(RespDeleteKategoriBarang resp) {
                ActivityKategoriBarang.navigate(ctx, id);
            }
