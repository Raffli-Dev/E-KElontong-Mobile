...
public class AdapterReturPenjualan extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public AdapterReturPenjualan(Context context, RecyclerView view, int page) {
        this.ctx = context;
        this.page = page;
    ...
    public class OriginalViewHolder extends RecyclerView.ViewHolder {
        public TextView name, txt_harga_jual, txt_brand, txt_kategori, tv_add_cart;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_ITEM) {
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.activity_retur_penjualan_row, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof OriginalViewHolder) {
            ...
            v.txt_brand.setText(product_list.brand_name);
    ...
    private void toggleCartButton(ProductList product_list, OriginalViewHolder v, Long product_id) {
        CartReturSales c = db.getCartReturSales(Long.parseLong(product_list.id));
        if (c != null) {
            product_id_db_sqlite = c.product_id;
        }
        if (product_id == product_id_db_sqlite ) {
            db.deleteActiveCartReturSales(Long.parseLong(product_list.id));

            CartReturSales cc = db.getCartReturSales(Long.parseLong(product_list.id));
            if (cc != null) {
                product_id_db_sqlite = c.product_id;
            }
            v.lyt_add_cart.setBackgroundColor(ctx.getResources().getColor(R.color.colorAddCart));
            v.tv_add_cart.setText("TAMBAH KE RETUR");
            Toast.makeText(ctx, "Remove From Retur Sales", Toast.LENGTH_SHORT).show();
            ...
            CartReturSales cart = new CartReturSales(Long.parseLong(product_list.id), product_list.item_name, product_list.item_image, 1, Long.parseLong(product_list.stock), selected_price, System.currentTimeMillis(), Long.parseLong(product_list.sales_id), Long.parseLong(product_list.sales_qty));
            db.saveCartReturSales(cart);
            Toast.makeText(ctx, "Add To Retur Sales Cart", Toast.LENGTH_SHORT).show();
//            Toast.makeText(ctx, "Produk Id : " + Long.parseLong(product_list.id)+", Name : "+ product_list.item_name+", Imafes" + product_list.item_image+", Stok : "+ Long.parseLong(product_list.stock)+", Selected Price " + selected_price, Toast.LENGTH_LONG).show();
    ...
    private void refreshCartButton(ProductList product_list, OriginalViewHolder v, Long product_idd) {
        ...
        if (product_idd == product_id_db_sqlite) {
            ...
            v.tv_add_cart.setText("HAPUS RETUR");
        } else {
            v.lyt_add_cart.setBackgroundColor(ctx.getResources().getColor(R.color.colorAddCart));
            v.tv_add_cart.setText("TAMBAH KE RETUR");
        }
