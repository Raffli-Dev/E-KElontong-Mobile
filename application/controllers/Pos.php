...
class Pos extends MY_Controller {
    ...
    public function print_invoice_kwitansi_pos($sales_id){

//        $companyDetails = $this->company_model->get_details();
//
//        $pdf = $this->SetUPPDF('surat_penerbitan_ijin_'.date("YmdHis"), 5, 5, 5);
//        $data = array(
...
//        $html = $this->load->view('sal-invoice-kwitansi-pos', $data, true);
////
//        $pdf->writeHTML($html, true, false, false, false, '');
//        $pdf->Output("Surat_Penerbitan_Ijin_".date("YmdHis").".pdf", 'I');
        ...
        $data['page_title']=$this->lang->line('sales_invoice');
        $data=array_merge($data,array('sales_id'=>$sales_id));
        $this->load->view('sal-invoice-kwitansi-pos',$data);

    }
    protected function SetUPPDF($title, $margin_left = 10, $margin_top = 20, $margin_right = 10) {
        ...
        $pdf->SetKeywords('Faktur Penjualan');

        $pdf->setPrintHeader(false);
        $pdf->setPrintFooter(false);
        $pdf->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
        $pdf->SetMargins($margin_left, $margin_top, $margin_right, true);
