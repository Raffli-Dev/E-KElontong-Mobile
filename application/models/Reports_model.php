...
//		$this->db->group_by("c.`item_id`");
    ...
    function _create_query_with_company_id($table_name,$table_column,$from_date,$to_date,$company_id){
        $ids = array();
        ...
        $this->db->where("(".$table_column.">='$from_date' and ".$table_column."<='$to_date' AND company_id = '$company_id' )");
    ...
    function _get_db_sales_ids_with_company_id($from_date,$to_date,$company_id){

        return $this->_create_query_with_company_id('db_sales','sales_date',$from_date,$to_date,$company_id);
    ...
    function _get_db_sales_return_ids_with_company_id($from_date,$to_date, $company_id){

        return $this->_create_query_with_company_id('db_salesreturn','return_date',$from_date,$to_date, $company_id);
    ...
    public function get_profit_loss_report_one_year(){
...
//        $to_date = date("Y-m-d", strtotime('+ 1days'));
        ...
        $sales_ids = $this->_get_db_sales_ids_with_company_id($from_date,$to_date, $company_id);
        $sales_return_ids = $this->_get_db_sales_return_ids_with_company_id($from_date,$to_date, $company_id);
        ...
        $this->db->select("COALESCE(SUM(b.qty*a.purchase_price),0) AS  opening_stock_price");
        $this->db->from("db_items AS a , db_stockentry AS b");
        $this->db->where("a.id=b.item_id")->where("company_id = '$company_id'");
        $opening_stock_price=$this->db->get()->row()->opening_stock_price;
        $info['opening_stock_price']=number_format($opening_stock_price,2);
        ...
        $this->db->select("COALESCE(SUM(a.purchase_price),0) AS total_purchase_price");
        $this->db->from("db_salesitems as a,db_sales as b");
        $this->db->where(" a.sales_id=b.id and b.sales_status='Final'");
        $this->db->where("b.company_id = '$company_id'");
        $this->db->where("(b.sales_date>='$from_date' and b.sales_date<='$to_date')");
        $total_purchase_price=$this->db->get()->row()->total_purchase_price;
        $info['total_purchase_price']=(number_format($total_purchase_price,2));

        $this->db->select("COALESCE(SUM(a.price_per_unit),0) AS total_price_per_unit");
        $this->db->from("db_salesitems as a,db_sales as b");
        $this->db->where(" a.sales_id=b.id and b.sales_status='Final'");
        $this->db->where("b.company_id = '$company_id'");
        $this->db->where("(b.sales_date>='$from_date' and b.sales_date<='$to_date')");
        $total_total_price_per_unit=$this->db->get()->row()->total_price_per_unit;
        $info['total_price_per_unit']=(number_format($total_total_price_per_unit,2));
        ...
        $total_laba_kotor = $sal_total - $total_purchase_price;
        $info['total_laba_kotor'] = (number_format($sal_total - $total_purchase_price,2)); ;
        ...
        $this->db->select("*");
        $this->db->from("db_expense a");
        $this->db->where("(a.expense_date>='$from_date' and a.expense_date<='$to_date')");
        $this->db->where("a.company_id = '$company_id'");
        $expenseList = $this->db->get()->result();
        $data_expense = array();
        $total_expense = 0;
        foreach ($expenseList as $itemExpense) {
            $row['id'] = $itemExpense->id;
            $row['expense_code'] = $itemExpense->expense_code;
            $row['category_id'] = $itemExpense->category_id;
            $row['expense_date'] = date('d F Y', strtotime($itemExpense->expense_date));
            $row['reference_no'] = $itemExpense->reference_no;
            $row['expense_for'] = $itemExpense->expense_for;
            $row['expense_amt'] = $itemExpense->expense_amt;
            $row['note'] = $itemExpense->note;
            $row['created_by'] = $itemExpense->created_by;
            $row['created_date'] = date('d F Y', strtotime($itemExpense->created_date));
            $row['created_time'] = $itemExpense->created_time;
            $row['system_ip'] = $itemExpense->system_ip;
            $row['system_name'] = $itemExpense->system_name;
            $row['status'] = $itemExpense->status;
            $row['company_id'] = $itemExpense->company_id;
            $total_expense += $itemExpense->expense_amt;
            $data_expense[] = $row;
        }
        $info['expense_list'] = $data_expense;

        $laba_usaha = $total_laba_kotor - $total_expense;
        $info['laba_usaha'] = (number_format($laba_usaha,2));
